...
public class SecurityConfig {
        ...
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/api/v1/auth/**",
                                ...
                                "/v3/api-docs/**",
                                "/oauth2/**",
                                "/api/v1/orders/webhook/payos",
                                "/api/v1/promotions/tiers")
                        .permitAll()
    ...
    JwtDecoder jwtDecoder() {
        ...
        OAuth2TokenValidator<Jwt> accessTokenOnlyValidator = jwt -> {
            boolean isAccessToken = Boolean.TRUE.equals(jwt.getClaimAsBoolean("isAccessToken"));

            if (!isAccessToken) {
                return OAuth2TokenValidatorResult.failure(
                        new OAuth2Error("invalid_token", "Token is not a valid access token", null));
            }
            return OAuth2TokenValidatorResult.success();
        };

        decoder.setJwtValidator(
                new DelegatingOAuth2TokenValidator<>(defaultValidators, accessTokenOnlyValidator));
